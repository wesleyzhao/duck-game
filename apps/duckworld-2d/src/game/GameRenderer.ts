...
export class GameRenderer {
  ...
  private sparkleContainer: Container | null = null
  private sparkleParticles: Graphics[] = []
  ...
  private readonly MOVE_SPEED = 24  // Super fast unicorn!
  ...
  async init(container: HTMLElement): Promise<void> {
    ...
    // Add sparkle container below player (trail effect)
    this.sparkleContainer = new Container()
    this.worldContainer.addChild(this.sparkleContainer)
  ...
  private gameLoop(): void {
    ...
    // Update jump physics
    useGameStore.getState().updateJump(deltaSeconds)
  ...
  private processMovement(): void {
    ...
    if (this.keysPressed.has(' ')) {
      ...
      this.keysPressed.delete(' ') // Prevent continuous jumping
    }
  ...
  private checkEnemyCollision(): void {
    ...
    // Don't check if jumping (can hop over enemies!)
    if (gameStore.isPlayerAirborne()) return
  ...
  private renderPlayer(): void {
    ...
    if (gameStore.isPlayerAirborne()) {
      const groundY = player.groundY + this.bobOffset
      const jumpHeight = groundY - y
      const shadowScale = Math.max(0.3, 1 - jumpHeight / 100) // Shadow shrinks as player rises

      // Draw ellipse shadow at ground level
      g.ellipse(x, groundY, 20 * scale * shadowScale, 8 * scale * shadowScale)
    ...
    switch (appearance.shape) {
      case 'unicorn':
        this.drawUnicorn(g, x, animY, scale, appearance.color)
        break
      case 'dinosaur':
  ...
  private drawUnicorn(g: Graphics, x: number, y: number, scale: number, color: string): void {
    // Main body (oval)
    ...
    // Front legs
    g.rect(x + bodyWidth * 0.15 - legWidth / 2, y + bodyHeight * 0.3, legWidth, legHeight)
    g.fill(color)
    g.rect(x + bodyWidth * 0.3 - legWidth / 2, y + bodyHeight * 0.3, legWidth, legHeight)
    g.fill(color)
    ...
    const hoofHeight = 3 * scale
    g.rect(x + bodyWidth * 0.15 - legWidth / 2, y + bodyHeight * 0.3 + legHeight - hoofHeight, legWidth, hoofHeight)
    g.fill('#333333')
    g.rect(x + bodyWidth * 0.3 - legWidth / 2, y + bodyHeight * 0.3 + legHeight - hoofHeight, legWidth, hoofHeight)
    g.fill('#333333')
    g.rect(x - bodyWidth * 0.15 - legWidth / 2, y + bodyHeight * 0.3 + legHeight - hoofHeight, legWidth, hoofHeight)
    g.fill('#333333')
    g.rect(x - bodyWidth * 0.3 - legWidth / 2, y + bodyHeight * 0.3 + legHeight - hoofHeight, legWidth, hoofHeight)
    ...
    // Snout/muzzle (small oval extending from head)
    g.ellipse(headX + headRadius * 0.8, headY + headRadius * 0.3, 6 * scale, 4 * scale)
    g.fill(color)

    // Horn (triangle on top of head) - golden/yellow
    const hornBaseX = headX
    const hornBaseY = headY - headRadius * 0.8
    const hornHeight = 18 * scale
    const hornWidth = 6 * scale
    g.moveTo(hornBaseX, hornBaseY)
    g.lineTo(hornBaseX + hornWidth / 2, hornBaseY - hornHeight)
    g.lineTo(hornBaseX + hornWidth, hornBaseY)
    g.closePath()
    g.fill('#FFD700') // Gold horn

    // Horn spiral lines (decorative)
    g.moveTo(hornBaseX + hornWidth * 0.2, hornBaseY - hornHeight * 0.2)
    g.lineTo(hornBaseX + hornWidth * 0.8, hornBaseY - hornHeight * 0.3)
    g.stroke({ width: 1, color: '#FFA500' })
    g.moveTo(hornBaseX + hornWidth * 0.25, hornBaseY - hornHeight * 0.5)
    g.lineTo(hornBaseX + hornWidth * 0.7, hornBaseY - hornHeight * 0.6)
    g.stroke({ width: 1, color: '#FFA500' })

    // Rainbow mane (flowing from head down neck)
    const maneColors = ['#FF0000', '#FF7F00', '#FFFF00', '#00FF00', '#0000FF', '#8B00FF']
    for (let i = 0; i < maneColors.length; i++) {
      const maneX = headX - headRadius * 0.5 - i * 3 * scale
      const maneY = headY - headRadius * 0.3 + i * 2 * scale
      const maneWidth = 8 * scale
      const maneHeight = 12 * scale + i * 3 * scale

      g.ellipse(maneX, maneY, maneWidth / 2, maneHeight / 2)
      g.fill(maneColors[i])
    }

    // Ear (small triangle)
    const earX = headX - headRadius * 0.3
    const earY = headY - headRadius * 0.9
    g.moveTo(earX, earY)
    g.lineTo(earX + 4 * scale, earY - 8 * scale)
    g.lineTo(earX + 8 * scale, earY)
    g.closePath()
    g.fill(color)

    // Inner ear (pink)
    g.moveTo(earX + 2 * scale, earY - 1 * scale)
    g.lineTo(earX + 4 * scale, earY - 5 * scale)
    g.lineTo(earX + 6 * scale, earY - 1 * scale)
    g.closePath()
    g.fill('#FFB6C1')

    // Eye (large, sparkly)
    const eyeX = headX + 4 * scale
    const eyeY = headY - 2 * scale
    g.circle(eyeX, eyeY, 4 * scale)
    g.fill('#000000')
    // Eye highlight
    g.circle(eyeX + 1.5 * scale, eyeY - 1.5 * scale, 1.5 * scale)
    g.fill('#FFFFFF')

    // Tail (rainbow flowing behind)
    const tailX = x - bodyWidth * 0.5
    const tailY = y - bodyHeight * 0.1
    const tailColors = ['#FF0000', '#FF7F00', '#FFFF00', '#00FF00', '#0000FF', '#8B00FF']

    for (let i = 0; i < tailColors.length; i++) {
      const tx = tailX - 5 * scale - i * 4 * scale
      const ty = tailY + i * 2 * scale
      // Wavy tail segments
      const wave = Math.sin(this.animationTime * 4 + i) * 3 * scale
      g.ellipse(tx, ty + wave, 6 * scale, 10 * scale)
      g.fill(tailColors[i])
    }
  }
  ...
  private spawnSparkle(x: number, y: number): void {
    ...
    const color = this.RAINBOW_COLORS[Math.floor(Math.random() * this.RAINBOW_COLORS.length)]
    ...
    // Store particle data
    ;(sparkle as any).lifetime = 0
    ;(sparkle as any).maxLifetime = 0.5 + Math.random() * 0.3
    ;(sparkle as any).velX = (Math.random() - 0.5) * 2
    ;(sparkle as any).velY = (Math.random() - 0.5) * 2 - 1 // Slight upward bias
    ;(sparkle as any).rotSpeed = (Math.random() - 0.5) * 0.2
  ...
  private updateSparkles(deltaTime: number): void {
    ...
    if (isMoving) {
      ...
      if (this.sparkleTimer > 0.05) { // Spawn every 50ms while moving
        this.spawnSparkle(player.x, player.y)
        this.sparkleTimer = 0
      }
    }
    ...
    for (let i = this.sparkleParticles.length - 1; i >= 0; i--) {
      ...
      sparkle.x += data.velX
      sparkle.y += data.velY
      sparkle.rotation += data.rotSpeed

      // Fade out based on lifetime
      const progress = data.lifetime / data.maxLifetime
      sparkle.alpha = 1 - progress
      sparkle.scale.set(1 - progress * 0.5) // Also shrink
  ...
  destroy(): void {
    ...
    if (this.sparkleContainer) {
      this.sparkleContainer.removeChildren()
    }
