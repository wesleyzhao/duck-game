...
export const useLevelStore = create<LevelStore>((set, get) => ({
  ...
  advanceLevel: () => {
    ...
    if (state.currentLevel < 3) {
      ...
      // Reset all trees to unsolved for the new level
      const gameStore = useGameStore.getState()
      gameStore.entities.forEach(entity => {
        if (entity.hasMathSymbol && entity.mathSolved) {
          gameStore.updateEntity(entity.id, { mathSolved: false })
        }
      })
